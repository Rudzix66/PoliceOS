      ...
      if (rows) {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          const fines = new Promise((res, rej) => {
            db.all(
              "SELECT * FROM fines WHERE userId = ?;",
              [parseInt(row.id)],
              function (err, result) {
                if (err) row.fines = 0;
                else row.fines = result.length;
                res();
              }
            );
          });
          const arrest = new Promise((res, rej) => {
            db.all(
              "SELECT * FROM arrest WHERE userId = ?;",
              [parseInt(row.id)],
              function (err, result) {
                if (err) row.arrest = 0;
                else row.arrest = result.length;
                if (i === rows.length - 1) console.log(null);
                res();
              }
            );
          });
...
app.get("/usersInfo", function (req, res) {
  ...
  console.log(id);
  if (Number(id)) query = { ...queries.user };
        ...
        if (err) {
          return res.send(
            response(400, {
              error: err,
              data: [userId, name, description, reason],
              query: `INSET INTO ${view} (userid,name,description,reason) VALUES (?,?,?,?);`,
            })
          );
        } else {
...
app.post("/users", (req, res) => {
  ...
  if (!action) return res.send(toJSON(code["400"]));
  ...
  if (action === "add") {
    ...
    db.run(
      queries.userAdd,
      [first_name, last_name, fullname, status, birth_date],
      function (err) {
        if (err) {
    ...
    if (!check) return;
    if (!available.indexOf(value)) return res.send(toJSON(code["400"]));
    db.run(
    ...
    const id = parseInt(data.id);
    if (!id) return res.send(toJSON(code["400"]));
